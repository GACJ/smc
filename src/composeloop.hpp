...
struct composeloop
{
    static void run(Composer& composer)
    {
        ...
        composeloop:
            ...
            if (CALLCOUNT)
            {
                ...
                eax = ((int*)ebx.ncallsleftperpos)[edx] - 1;
                if (eax < 0)
                {
                    ebx.ncallsleft[ecx]++;
                    goto lbl_sublennextcall;
                }
                ((int*)ebx.ncallsleftperpos)[edx] = eax;
            }
    ...
    static bool backtrack_sub(Composer& ebx, Composition*& edi, Node*& esi, int& ecx, int& ebp)
    {
        ...
        } while ((MULTIPART && ebx.maxpartlength == ebp) || lbl_nextcall2(ebx, esi, ecx));
        STOPTIMEBT;
        return false;
    }

    static bool lbl_nextcall2(Composer& ebx, const Node* esi, int& ecx)
    {
        while (true)
        {
#ifdef REVERSECALLS
            ...
            else
            {
                if (MULTIPART && CALLCOUNT)
                {
                    ...
                    auto edx = ecx + esi->nextnode[ecx]->callcountposindex;
