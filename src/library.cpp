...
int Composer::storecomp2(CompMusicStore& storedcomp)
{
    ...
    author = nullptr; // Unlinks author from Composer!!
    return (TRUE);
}
...
int Composer::loadcomposition(CompMusicStore& storedcomp)
{
    ...
    storedcomp.author = nullptr; // Author unlinked from storedcomp!
...
int FragmentLibrary::add(Fragment& newfrag, int endplacebell)
{
    ...
    if (fragmap[endplacebell] == nullptr)
    {
        ...
        for (i = 0; i < mapsize; i++)
            fragmap[endplacebell][i].bulklist = nullptr;
    }
...
char* Composer::writepattern(char* buf, Pattern* pattern, int length, int startplacebell)
{
    ...
    *buf = 0; // Need to nullptr-terminate after printcall()
    return (buf);
}
...
// PRIMARY pattern. Otherwise, returns nullptr
