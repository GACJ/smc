...
// Find startnode
 if (startnode==NULL && samerow(startrow,nodex->nodehead))
  startnode = node;
...
// Copy duplicate fields out of NodeExtra structure
 ...
 else
 {
  ...
  for (i=0; i<nodex->nfalsenodes; i++)
   node->falsenodes[i] =
   	(Node *)(nodes+nodex->falsenodes[i]*nodesize);
 }
