...
// Output checkpoint stats - use values saved when last comp was produced
  sprintf(outfile.buffer,"%u %u %u %u %u %u %u %d ",
 	(int)stats.save_ncompsfound, ((int *)&stats.save_ncompsfound)[1],
 	(int)stats.save_nrotsfound, ((int *)&stats.save_nrotsfound)[1],
 	(int)stats.save_nodesgenerated, ((int *)&stats.save_nodesgenerated)[1],
 	(int)stats.save_elapsed,((int *)&stats.save_elapsed)[1]);
...
void Composer::finaloutput()
{
 ...
 sprintf(outfile.buffer,"%d.%03d million nodes generated",
 	int(stats.nodesgenerated/1000000),(int)(stats.nodesgenerated%1000000)/1000);
 printf("%s\n",outfile.buffer);
 outfile.writeline();
 ...
 printf("%s\n",outfile.buffer);
 outfile.writeline();
 sprintf(outfile.buffer,"Best score %d, longest length %d",
 	stats.bestscore,stats.longestlength);
 printf("%s\n",outfile.buffer);
 outfile.writeline();
...
// Print library stats
 ...
 if (makefraglib)
 {
  sprintf(outfile.buffer,"%d comps in hashtable; %d out of %d entries used",
  	comphasher.nitems,comphasher.entriesused,comphasher.gettablesize());
  printf("%s\n",outfile.buffer);
  outfile.writeline();
...
Composer::writeheaderessential(LineFile &f)
{
 ...
 for (i=0; i<nbells; i++)
  if (i<7-1)
   ...
   tmprow[i] = i;
 if (!coursestructured || callingbell!=nbells-1 || courseend.nrows>1 ||
 	!samerow(courseend.matches[0].row,tmprow))
 {
...
// Stop when a MUSIC section is reached
  ...
  {
   musicinclude = strtok(NULL,DELIMITERS);
   break;
  }
...
// Read MUSTHAVE/SHOULDHAVE blocks
  else if (strncmpi(p,TOKEN_MUSTHAVE,strlen(TOKEN_MUSTHAVE))==0 ||
  	strncmpi(p,TOKEN_SHOULDHAVE,strlen(TOKEN_SHOULDHAVE))==0)
  {
...
// of a search file
  ...
  if (p)
  {
   ...
   else if (weight<0)		// Weighting < 0?
   {
    ...
    {
     nmusicdefs--;
     while (strtok(NULL,DELIMITERS))	// Count number of excluded rows
      exclude.nrows++;
    }
   }
  }
...
Composer::readrowmatch(char *buffer,MusicRow &m)
{
 ...
 for (i=0; i<nbells && buffer[i]; i++)
  if (strchr(MATCHWILD,buffer[i])==NULL)
  {
...
// For wrap rows, the backstroke change must be read into wrapbackstroke
  ...
  for (j=0; j<nbells && buffer[i+j+1]; j++)
   if (strchr(MATCHWILD,buffer[i+j+1])==NULL)
   {
...
// Parse checkpoint composition
 if (inputcomp(outfile.buffer+1)==NULL)
  return(FALSE);
// Read restart stats values
 ...
 if (8!=sscanf(outfile.buffer,"%u %u %u %u %u %u %d %u",
 	(int *)&stats.ncompsfound,((int *)&stats.ncompsfound)+1,
 	(int *)&stats.nrotsfound,((int *)&stats.nrotsfound)+1,
 	(int *)&stats.nodesgenerated,((int *)&stats.nodesgenerated)+1,
 	(int *)&stats.elapsed,((int *)&stats.elapsed)+1))
 {
